(function () {
  ...
  function EqualizerSliders(props) {
    ...
    var draggingBand = React.useRef(null);
    ...
    function setBandFromPointer(key, event) {
      var hitbox = event.currentTarget.querySelector(".hermes-amp-eq-hitbox");
      if (!hitbox) return;
      var rect = hitbox.getBoundingClientRect();
      var ratio = clamp((rect.bottom - event.clientY) / Math.max(rect.height, 1), 0, 1);
      var value = Math.round((ratio * 200) / 5) * 5;
      var input = event.currentTarget.querySelector(".hermes-amp-eq-range");
    ...
    function startPointerDrag(key, event) {
      if (event.button !== undefined && event.button !== 0) return;
      event.preventDefault();
      draggingBand.current = key;
      if (event.currentTarget.setPointerCapture) {
        try {
          event.currentTarget.setPointerCapture(event.pointerId);
        } catch (error) {}
      }
      setBandFromPointer(key, event);
    }

    function movePointerDrag(key, event) {
      if (draggingBand.current !== key) return;
      event.preventDefault();
      setBandFromPointer(key, event);
    }

    function stopPointerDrag(key, event) {
      if (draggingBand.current !== key) return;
      event.preventDefault();
        ...
        MIX_BANDS.map(function (band) {
          ...
          return e(
            "label",
            {
              ...
              onPointerDown: function (event) { startPointerDrag(band.key, event); },
              onPointerMove: function (event) { movePointerDrag(band.key, event); },
              onPointerUp: function (event) { stopPointerDrag(band.key, event); },
              onPointerCancel: function (event) { stopPointerDrag(band.key, event); },
              onLostPointerCapture: function (event) { stopPointerDrag(band.key, event); }
            },
            e("span", { className: "hermes-amp-eq-hitbox", "aria-hidden": "true" }),
            e("input", {
